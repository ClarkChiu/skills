...
- `rtk`: portable **setup + reference** for the user's RTK (Rust Token Killer) CLI — carries the (previously machine-local) RTK reference into the repo and wires the `rtk hook claude` PreToolUse hook into settings.json (merge-not-clobber, coexists with `git-guardrails`). APM carries the config/reference; the rtk *binary* is a per-machine install the skill is honest about not carrying. Original (wraps the user's own CLI) — no attribution/sources.lock. Note: `terse` (in-context brevity) and `rtk` (CLI-output token proxy) both save tokens but at different layers — terse trims the model's prose, rtk compresses tool output.
...
| `git-guardrails` | (none — it's a hook) | a PreToolUse safety hook, not a prose skill; no built-in equivalent. Merges alongside the RTK hook |
