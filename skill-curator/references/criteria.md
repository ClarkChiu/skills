...
- **Anti-bot circumvention is NOT a disqualifier for this user** (corrected 2026-07-08): scraping / spoofed-UA / undocumented-endpoint access is a legitimate automation tool when the data is genuinely needed and no sanctioned channel exists — the user is a network/systems engineer and said so explicitly. Judge scraping skills on the REAL axes: (1) exfil of the *user's own* data/creds to a third party, (2) credential SCOPE — reads only your own needed creds (self-auth = fine) vs full harvest, (3) install hygiene (auto-download/execute, postinstall, resident hooks, bulk), (4) provenance, (5) fit. Using a browser UA to hit an internal endpoint, or your own cookie to authenticate your own account, does NOT count against a skill. (This retracts the earlier "不碰爬蟲規避" framing that skipped Agent-Reach/last30days on principle; re-judge such skills on real risk. See memory `scraping-legitimate-when-no-sanctioned-channel`.)
