...
- 自己寫的技能裡，凡是中文的內容（`SKILL.md`、`references/` 規則檔、評測 `evals.json` 的說明文字等）都用清楚的臺灣繁體中文，不要中英夾雜。能用中文就用中文：進行式寫「進行式」不要寫 `-ing`、技能寫「技能」不要寫 `skill`、基準寫「基準」不要寫 `baseline`、斷言寫「斷定」不要寫 `assert`。例外是程式識別字、專有名詞（產品名、人名）、JSON 欄位名、以及測試用的輸入樣本，這些是資料不是敘述文字，維持原樣。
