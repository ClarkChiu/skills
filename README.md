...
| [`skill-evolve`](./skill-evolve/) | 隨叫隨跑（或排程）的**自我維護偵察**，兩條證據流：(1) **上游漂移**——掃自寫 skill 的 `references/attribution.md` 找出上游來源，用 GitHub API 比對各 skill 的 `sources.lock` 基線看有無更新，並擴展搜尋新專案；(2) **你自己的使用**——用 `mine_usage.py`（唯讀、無網路）挖 Claude Code 對話紀錄，找出重複手做卻沒 skill 的事（GAP）、被繞過的技能（FRICTION）、該進記憶的偏好（MEMORY）。兩流合成一份報告**與你討論**——只偵察與建議，**從不自動改 skill**。**僅適用於本專案自寫的 skill**；外部（經 APM 引入）的技能用 `apm install` 隨上游更新即可，不歸它管。採納任何新來源前先過 `skill-auditor`。需本機 Python；上游比對建議設 `GITHUB_TOKEN`（公開讀取權限即可）以免 API 限流。 |
